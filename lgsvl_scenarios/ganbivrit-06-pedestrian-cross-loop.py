...
for loop_index in range(params.loop_count):
    ...
    # state_ego.velocity = 0.5 * forward
    ...
    # add pedestrian
    start = zero_pose + 10*forward - 6*right + randoms[2] * right + randoms[3] * forward
    ped = sim.add_agent( random.choice(simulator_types.pedestrian_types), lgsvl.AgentType.PEDESTRIAN, 
        lgsvl.AgentState(transform = Transform(position = start, rotation = simulator_types.positions.ped_rotation_90_ganheb - 180)))

    #ped_velocity = input("Input pedestrian speed and Press Enter to run the simulation for 60 seconds:")
    ped_velocity = 2 * randoms[4]

    end = start + 25*right

    wp = [lgsvl.WalkWaypoint(start, 0, 0, ped_velocity), lgsvl.WalkWaypoint(end, 0, 0, ped_velocity), lgsvl.WalkWaypoint(start, 0, 0, ped_velocity)]
    ped.follow(wp, True)
    # Add Cone to Map 
