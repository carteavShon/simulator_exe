...
for loop_index in range(params.loop_count):
    ...
    # ego = sim.add_agent(env.str("LGSVL__VEHICLE_0", "909be5bb-cfa4-43af-b1f6-5e8780d63382"), lgsvl.AgentType.EGO, state)
    ...
    stateZoe.transform.position = (zero_pose  + 5*forward + 2*right) + randoms[2] * right + randoms[3] * forward
    z = sim.add_agent("Zoe", lgsvl.AgentType.PEDESTRIAN, stateZoe)

    stateHoward = lgsvl.AgentState()
    stateHoward.transform.position =  (zero_pose  + 12*forward + 10*right) + randoms[4] * right + randoms[5] * forward
