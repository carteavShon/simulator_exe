...
scenario_num = 16
scenario_name = "Vehicle rides along WG-Bells."
...
for loop_index in range(params.loop_count):
    ...
    wp = sim_pois.get_drive_points(sim_pois.WG_Bells_Path(sim)[3:], 1.9*randoms[4], state_ego.rotation.y)
    wp = wp[2:]
    
    npc_state = lgsvl.AgentState(
            transform = Transform(
                rotation = Vector( x= state_ego.rotation.x, y = state_ego.rotation.y, z= state_ego.rotation.z), 
                position = wp[0].position))  
